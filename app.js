...
async function initTenderPage() {
  ...
  function renderView(rows) {
    ...
    if (gaugesEl) {
      if (!ongoingForGauges.length) {
        ...
        gaugesEl.innerHTML=ongoingForGauges.map((r,i)=>{
          ...
          // SVG half-circle: cx=60,cy=62,r=48; arc left→right
          const cx=60,cy=62,rad=48;
          const bgArc=`M ${cx-rad} ${cy} A ${rad} ${rad} 0 0 1 ${cx+rad} ${cy}`;
          let fillArc='', ex=cx-rad, ey=cy;
          if (displayPct>0) {
            const angle=Math.PI-(displayPct/100)*Math.PI;
            ex=cx+rad*Math.cos(angle); ey=cy-rad*Math.sin(angle);
            fillArc=`M ${cx-rad} ${cy} A ${rad} ${rad} 0 ${displayPct>50?1:0} 1 ${ex.toFixed(2)} ${ey.toFixed(2)}`;
          }

          // Tick marks at 25%, 50%, 75%, 100%
          const ticks=[25,50,75,100].map(tp=>{
            const ta=Math.PI-(tp/100)*Math.PI;
            const inner=rad-7, outer=rad+2;
            const x1=cx+inner*Math.cos(ta), y1=cy-inner*Math.sin(ta);
            const x2=cx+outer*Math.cos(ta), y2=cy-outer*Math.sin(ta);
            return `<line x1="${x1.toFixed(1)}" y1="${y1.toFixed(1)}" x2="${x2.toFixed(1)}" y2="${y2.toFixed(1)}" stroke="rgba(255,255,255,.18)" stroke-width="1.2"/>`;
          }).join('');
          ...
          // ── Needle arrow: pivots from cx,cy pointing along the arc angle ──
          // Needle angle: 180° = 0%, 0° = 100% (left→right sweep)
          const needleAngle = Math.PI - (displayPct / 100) * Math.PI; // radians
          const needleLen = rad - 6;   // tip reaches near arc
          const needleTipX  = cx + needleLen * Math.cos(needleAngle);
          const needleTipY  = cy - needleLen * Math.sin(needleAngle);
          // Arrow-head: two side points perpendicular to needle, 5px back from tip
          const arrowSize = 5.5;
          ...
          return `<div style="text-align:center;min-width:148px;${isExpired?'animation:expiredPulse 1.2s ease-in-out infinite;':''}" title="${r['Tender No']||''} · ${poleType} · Limit: ${limitDays}d">
            <svg viewBox="0 0 120 72" width="148" height="88" style="overflow:visible">
              <defs>
                ...
                ${isExpired?`<filter id="glow-${i}"><feGaussianBlur stdDeviation="2.5" result="coloredBlur"/><feMerge><feMergeNode in="coloredBlur"/><feMergeNode in="SourceGraphic"/></feMerge></filter>`:''}
              </defs>
              <!-- Track -->
              <path d="${bgArc}" fill="none" stroke="rgba(255,255,255,0.06)" stroke-width="11" stroke-linecap="round"/>
              <!-- Ticks -->
              ${ticks}
              <!-- Fill arc -->
              ${fillArc?`<path d="${fillArc}" fill="none" stroke="url(#${gmId})" stroke-width="11" stroke-linecap="round" ${isExpired?`filter="url(#glow-${i})"`:''}/>`:''}
              <!-- Needle stem -->
              <path d="${needleStem}" stroke="${col}" stroke-width="2" stroke-linecap="round" opacity="0.85"/>
              <!-- Needle arrow-head -->
              <path d="${arrowHead}" fill="${col}" ${isExpired?`filter="url(#glow-${i})"`:''} style="filter:drop-shadow(0 0 ${isExpired?5:2}px ${col})"/>
              <!-- Pivot circle at base -->
              <circle cx="${cx}" cy="${cy}" r="4" fill="${col}" stroke="rgba(0,0,0,.5)" stroke-width="1.5"/>
              <!-- Age value center -->
              <text x="60" y="52" text-anchor="middle" font-size="14" font-weight="800" fill="${col}" font-family="monospace">${Math.round(ageDays)}d</text>
              <text x="60" y="63" text-anchor="middle" font-size="7" fill="rgba(255,255,255,.4)" font-family="sans-serif">of ${limitDays}d limit</text>
              <!-- 0 / limit labels -->
              <text x="${cx-rad-4}" y="${cy+12}" text-anchor="middle" font-size="7" fill="rgba(255,255,255,.3)">0</text>
              <text x="${cx+rad+4}" y="${cy+12}" text-anchor="middle" font-size="7" fill="rgba(255,255,255,.3)">${limitDays}</text>
            </svg>
            <div style="font-size:.64rem;font-weight:700;color:${col};margin-top:-.2rem;letter-spacing:.4px">${statusLabel}</div>
    ...
    rows._renderer=r=>{
      ...
      // Expire Date Age from sheet col L + pole-type limit
      const ageDays=parseFloat(r['Expire Date Age']||r['Expire Age']||r['Age']||0)||0;
      const limitDays=getPoleLimit(r['Pole Type']||'');
      const daysLeft=limitDays-ageDays;
      const isExpired=ageDays>limitDays;
      const nearExpiry=!isExpired&&daysLeft<=7;

      let expAgeTxt, expAgeCls;
      if (!ageDays && ageDays!==0) {
        expAgeTxt='—'; expAgeCls='text-muted';
      } else if (isExpired) {
        expAgeTxt=`${Math.round(ageDays)}d / ${limitDays}d ⚠`; expAgeCls='text-red';
      } else if (nearExpiry) {
        expAgeTxt=`${Math.round(ageDays)}d / ${limitDays}d`; expAgeCls='text-amber';
      } else {
        expAgeTxt=`${Math.round(ageDays)}d / ${limitDays}d`; expAgeCls='text-green';
      }

      // Inline mini half-circle badge with arrow needle for the table
      const miniPct=Math.min(100,ageDays/limitDays*100);
      const miniCol=isExpired?'#f43f5e':nearExpiry?'#f59e0b':miniPct>=70?'#f59e0b':'#10b981';
      const miniR=13,mCx=16,mCy=16;
      const mAngle=Math.PI-(miniPct/100)*Math.PI;
      const mEx=mCx+miniR*Math.cos(mAngle), mEy=mCy-miniR*Math.sin(mAngle);
      const miniFill=miniPct>0?`<path d="M ${mCx-miniR} ${mCy} A ${miniR} ${miniR} 0 ${miniPct>50?1:0} 1 ${mEx.toFixed(1)} ${mEy.toFixed(1)}" fill="none" stroke="${miniCol}" stroke-width="3" stroke-linecap="round"/>`:'';
      // Mini arrow needle
      const mNeedleLen=miniR-2;
      const mNTipX=mCx+mNeedleLen*Math.cos(mAngle), mNTipY=mCy-mNeedleLen*Math.sin(mAngle);
      const mBackX=mCx+(mNeedleLen-3)*Math.cos(mAngle), mBackY=mCy-(mNeedleLen-3)*Math.sin(mAngle);
      const mPerpX=Math.sin(mAngle)*2.2, mPerpY=Math.cos(mAngle)*2.2;
      const mArrTip=`${mNTipX.toFixed(1)},${mNTipY.toFixed(1)}`;
      const mArrL=`${(mBackX-mPerpX).toFixed(1)},${(mBackY+mPerpY).toFixed(1)}`;
      const mArrR=`${(mBackX+mPerpX).toFixed(1)},${(mBackY-mPerpY).toFixed(1)}`;
      const miniSvg=`<svg viewBox="0 0 32 18" width="36" height="22" style="vertical-align:middle;margin-right:4px">
        <path d="M ${mCx-miniR} ${mCy} A ${miniR} ${miniR} 0 0 1 ${mCx+miniR} ${mCy}" fill="none" stroke="rgba(255,255,255,0.08)" stroke-width="3" stroke-linecap="round"/>
        ${miniFill}
        ${miniPct>0?`<path d="M ${mCx} ${mCy} L ${mBackX.toFixed(1)} ${mBackY.toFixed(1)}" stroke="${miniCol}" stroke-width="1.3" stroke-linecap="round"/>
        <path d="M ${mArrTip} L ${mArrL} L ${mArrR} Z" fill="${miniCol}"/>
        <circle cx="${mCx}" cy="${mCy}" r="2" fill="${miniCol}" stroke="rgba(0,0,0,.4)" stroke-width="0.8"/>` :''}
      </svg>`;
      ...
      return `<tr${isExpired?' class="row-expired"':''}>
        ...
        <td class="num">${miniSvg}<span class="${expAgeCls}" style="font-size:.75rem;font-weight:600">${expAgeTxt}</span></td>
        <td class="num ${expAgeCls}" style="font-weight:600">${isExpired?'<span style="animation:expiredPulse 1s infinite">⚠ OVER</span>':daysLeft>0?Math.round(daysLeft)+'d left':'—'}</td>
        ...
        <div class="expand-content">
          ...
          <div class="expand-field"><div class="expand-field-label">Expire Date Age</div><div class="expand-field-val ${expAgeCls}">${Math.round(ageDays)} days used of ${limitDays}-day limit ${isExpired?'— ⚠ EXCEEDED':nearExpiry?'— expiring soon':''}</div></div>
          <div class="expand-field"><div class="expand-field-label">Pole Type Limit</div><div class="expand-field-val">${limitDays} days (${r['Pole Type']||'—'})</div></div>
          ${isOng?`
...
async function initFillingPage() {
  ...
  rows._renderer = r => {
    ...
    return `<tr>
      ...
      <td class="num">${totVL.toFixed(2)}</td>
      <td class="num text-green">Rs. ${fmtLKR(totRL)}</td>
    </tr><tr class="expand-row" style="display:none"><td colspan="12">
      <div class="expand-content">
